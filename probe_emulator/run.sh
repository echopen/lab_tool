...
while getopts "h m:" opt  #getopts search for option -h and -m + the argument ($OPTARG) of -m (because m:)
do
	case $opt in
		h)
			usage
...
case $machine in
	RP)
		...
		if [ -e probe_emulator ]; then # if the file exist
			rm probe_emulator
		fi
	...
	PC)
		...
		;;
	*)
		echo "wrong argument for option -m"
		exit 1
		;;
esac
